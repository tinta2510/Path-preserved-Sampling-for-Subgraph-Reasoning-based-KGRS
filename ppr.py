...
def get_ppr(loader, bs=128, N=15):
    ...
    for i in tqdm(range(n_batch)):
        ...
        u_index = torch.cat((torch.LongTensor(u_list).view(1,-1), torch.arange(tbs).view(1,-1)), dim = 0).to(loader.device)
        u_value = torch.ones(tbs).to(loader.device)
        U = torch.sparse_coo_tensor(u_index, u_value, (loader.n_nodes,tbs)).to(loader.device)
        ...
        for j in range(tbs):
            ...
            if j == 0:
                p_index = torch.cat((torch.arange(loader.n_nodes).view(1,-1), torch.zeros(1,loader.n_nodes)), dim = 0).to(loader.device)
                p_value = torch.zeros(1,loader.n_nodes).to(loader.device)
                p_value = p_value + (1 - beta) / (loader.n_nodes - len(p_list))
                p_value[0,p_list] = beta / len(p_list)  if len(p_list)!= 0 else 0
            else:
                p_id = torch.cat((torch.arange(loader.n_nodes).view(1,-1), j*torch.ones(1,loader.n_nodes)), dim = 0).to(loader.device)
                p_index = torch.cat((p_index,p_id), dim = 1).to(loader.device)
                p_val = torch.zeros(1,loader.n_nodes).to(loader.device)
                p_val = p_val + (1 - beta) / (loader.n_nodes - len(p_list))
                p_val[0,p_list] = beta / len(p_list)    if len(p_list)!= 0 else 0
                p_value = torch.cat((p_value, p_val), dim = 1).to(loader.device)
        p_value = p_value.squeeze(0)
        P = torch.sparse_coo_tensor(p_index, p_value, (loader.n_nodes,tbs)).coalesce().to(loader.device)
